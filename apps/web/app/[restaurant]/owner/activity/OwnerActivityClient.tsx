...
export function OwnerActivityClient({
  ...
  const totalSales = sessions.reduce((sum, s) => sum + (s.orders?.total || 0), 0)
  const activeSales = activeSessions.reduce((sum, s) => sum + (s.orders?.total || 0), 0)
    ...
    <div className="p-4 sm:p-8">
      ...
      <div className="mx-auto mb-8 flex max-w-7xl items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Monitor live tables and sales.</p>
        </div>
            ...
            <section className="mb-8 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
              <div className="rounded-2xl border border-border bg-card p-5 shadow-sm">
                <div className="mb-3 flex items-center justify-between text-muted-foreground">
                  <span className="text-xs font-bold tracking-wider uppercase">Live Tables</span>
                  <Activity className="h-5 w-5 text-primary" />
                </div>
                <h3 className="text-3xl font-extrabold text-foreground">{activeSessions.length}</h3>
                <p className="mt-1 text-xs text-muted-foreground">Currently ordering</p>
              </div>

              <div className="rounded-2xl border border-border bg-card p-5 shadow-sm">
                <div className="mb-3 flex items-center justify-between text-muted-foreground">
                  <span className="text-xs font-bold tracking-wider uppercase">Active Sales</span>
                  <Sparkles className="h-5 w-5 text-amber-500" />
                </div>
                <h3 className="text-3xl font-extrabold text-foreground">{symbol}{activeSales}</h3>
                <p className="mt-1 text-xs text-muted-foreground">In active sessions</p>
              </div>

              <div className="rounded-2xl border border-border bg-card p-5 shadow-sm">
                <div className="mb-3 flex items-center justify-between text-muted-foreground">
                  <span className="text-xs font-bold tracking-wider uppercase">Total Sales</span>
                  <TrendingUp className="h-5 w-5 text-green-500" />
                </div>
                <h3 className="text-3xl font-extrabold text-foreground">{symbol}{totalSales}</h3>
                <p className="mt-1 text-xs text-muted-foreground">All sessions today</p>
              </div>

              <div className="rounded-2xl border border-border bg-card p-5 shadow-sm">
                <div className="mb-3 flex items-center justify-between text-muted-foreground">
                  <span className="text-xs font-bold tracking-wider uppercase">Closed Sessions</span>
                  <CheckCircle className="h-5 w-5 text-muted-foreground/60" />
                </div>
                <h3 className="text-3xl font-extrabold text-foreground">{closedSessions.length}</h3>
                <p className="mt-1 text-xs text-muted-foreground">Completed checkouts</p>
              </div>
            </section>
                    ...
                    >
                      <div>
                        ...
                        <div className="mb-4 flex items-start justify-between border-b border-border pb-4">
                          <div>
                            <div className="flex items-center gap-2">
                              <h3 className="text-xl font-bold">Table {session.table_number}</h3>
                              <span
                            ...
                            <p className="mt-0.5 text-[10px] font-medium text-muted-foreground">
                              {isActive ? (
                                <>Opened <ElapsedTime createdString={session.created_at} /></>
                              ) : (
                          ...
                          <div className="text-right">
                            <span className="block text-xs font-bold text-muted-foreground uppercase">Total</span>
                            <span className="text-lg font-extrabold">{symbol}{orders.total || 0}</span>
                          </div>
                        </div>
                        ...
                        <div className="mb-6">
                          ...
                          {items.length === 0 ? (
                            <p className="py-2 text-xs text-muted-foreground italic">No items ordered.</p>
                          ) : (
                            <div className="max-h-48 space-y-2.5 overflow-y-auto pr-1">
                              {items.map((item: { item_id: string; qty: number; notes?: string; [key: string]: unknown }) => {
                                ...
                                return (
                                  <div key={item.item_id + (item.notes || "")} className="flex items-start justify-between text-xs">
                                    <div className="pr-4">
                                      <p className="font-semibold text-foreground">
                                        {name}
                                        <span className="ml-1.5 font-bold text-primary">x{item.qty}</span>
                                      </p>
                                      {item.notes && <p className="mt-0.5 text-[10px] text-muted-foreground italic">"{item.notes}"</p>}
                                    </div>
                      ...
                      <div className="mt-auto border-t border-border pt-4">
                        <div className="mb-4 grid grid-cols-2 gap-y-1 text-[11px] text-muted-foreground">
                          <div>Subtotal:</div>
                          <div className="text-right font-medium">{symbol}{orders.subtotal || 0}</div>
                          <div>Tax (10%):</div>
                          <div className="text-right font-medium">{symbol}{orders.tax || 0}</div>
                          {Number(orders.tips) > 0 && (
                            <>
                              <div>Tips:</div>
                              <div className="text-right font-semibold text-primary">+{symbol}{orders.tips}</div>
                            </>
                          )}
                        </div>
                        ...
                        <div className="flex w-full items-center justify-center gap-1.5 rounded-2xl bg-muted px-4 py-2 text-xs font-bold text-muted-foreground select-none">
                          {isActive ? "Use Scanner to Finalize" : "Session Finalized"}
                        </div>
                      </div>
                    </div>
                  )
