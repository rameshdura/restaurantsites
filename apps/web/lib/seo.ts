...
export function generateHomeMetadata(
  ...
  return {
      ...
      images: [
        {
          ...
          height: 630,
          alt:
            data.images?.heroImage?.alt ||
            `${data.name} - Authentic dining experience`,
        },
      ],
    ...
    twitter: {
      card: (data.social?.twitterCard || "summary_large_image") as
        | "summary_large_image"
        | "summary"
        | "player"
        | "app",
...
export function generateAboutMetadata(
  ...
  const title = data.seo?.aboutTitle || `About ${data.name}`

  const description =
    data.seo?.aboutDescription ||
    truncate(
      data.about?.content ||
        `Discover the story of ${data.name}. Learn about our heritage, culinary philosophy, and the team behind our authentic cuisine.`,
      160
    )
      ...
      images: [
        {
          ...
          height: 630,
          alt:
            data.images?.coverImage?.alt ||
            `${data.name} - Our story and heritage`,
        },
      ],
      type: "article",
    },
...
export function generateMenuMetadata(
  ...
  const cuisineTypes =
    data.schema?.servesCuisine?.slice(0, 3).join(", ") || "Japanese cuisine"
  const popularDishes = (data.menu || [])
    .filter((item) => item.isPopular)
    .slice(0, 3)
    .map((item) => item.name)
  ...
  const title =
    data.seo?.menuTitle || `${data.name} Menu | ${cuisineTypes} & Prices`

  const baseDescription =
    `Explore ${data.name}'s complete menu featuring ${cuisineTypes}. ` +
...
export function generateBrandMetadata(
  ...
  const title =
    data.seo?.brandTitle || `${data.name} Brand Assets | Marketing Materials`

  const description =
    data.seo?.brandDescription ||
...
export function generateCompanyMetadata(
  ...
  const title =
    data.seo?.companyTitle || `${companyName} | Corporate Information`

  const description =
    data.seo?.companyDescription ||
 ...
 */
export function generateListingMetadata(restaurantCount: number): Metadata {
  return {
    ...
    openGraph: {
      ...
      description:
        "Discover authentic dining experiences at our partner restaurants.",
      url: DOMAIN,
...
export function generateMenuSchema(
  ...
  const menuSections = (data.menu || []).reduce(
    (acc, item) => {
  ...
  const hasMenuSection = Object.entries(menuSections).map(
    ([category, items]) => ({
...
export function generateAboutPageSchema(
  ...
  if (data.foundingDate || data.about?.foundedYear) {
    mainEntity.foundingDate =
      data.foundingDate || `${data.about?.foundedYear}-01-01`
  }
...
export function generateOrganizationSchema(
  ...
  const schema: Record<string, unknown> = {
    ...
    address: {
      ...
      addressRegion: info?.address?.includes("Kanagawa")
        ? "Kanagawa"
        : info?.address?.includes("Tokyo")
          ? "Tokyo"
          : undefined,
